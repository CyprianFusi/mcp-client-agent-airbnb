...
# Set the event loop policy to WindowsProactorEventLoopPolicy for Windows subprocess support
...
server_params_2 = StdioServerParameters(
    command="uv",
    args=["--directory", "./", "run", "server.py"],  # Change to your memory server
)

async def run():
    """Main function to run the chat client with two MCP servers."""
    try:
